...
**多主节点复制**：每个主节点都可以接受写操作。处理写的每个主节点都必须将数据更改转发到所有其它节点。此时，每个主节点同时扮演其它主节点的从节点。

#### 适用场景

**在一个数据中心内部使用多主节点没有太大意义**，其复杂性已经超过其所能带来的好处。但是在多数据中心、离线客户端操作有其适用性。

##### 场景一：多数据中心

在每个数据中心内采用常规的主从复制方案，而在数据中心之间，由各个数据中心的主节点来负责同其它数据中心的主节点进行数据的交换。
...
**处理冲突最理想的策略是避免冲突发生。**如果应用层可以保证对特定记录的写请求总是通过一个主节点，这样就不会发生写冲突。

写冲突的解决方案：

1. 收敛于一致状态。

   所有的复制模型必须确保数据在所有副本最终状态一定是一致的。实现收敛的冲突解决方案可以采用如下方法：

   a 给每个写入分配一个ID，ID最大写入成功；
...
一些数据存储系统选择放弃主节点，允许任何副本直接接受来自客户端的写请求。比如亚马逊Dynamo、Riak、Cassandra等。

##### 数据写入与读取大概思路

客户端将写请求发送到多副本，有部分副本（w+r>n）确认写入，则写入成功；

客户端将读请求发送到多副本，多个副本都返回响应，采用版本号技术确定要读取哪写数据。

##### 无主复制面临的问题

###### 问题1:当一个失效的节点重新加入集群后，如何同步它错过的写请求？

方法1:读修复。当客户端并行读取多个副本时，可以检测到过期的返回值，客户端判断过期值，然后将新值写入到该副本。这种方法适合被频繁读取的场景。

方法2:反熵过程。一些数据存储有后台进程不断查找副本之间数据的差异，将任何缺少的数据从一个副本复制到另一个副本。此反熵过程并不保证特定的顺序复制写入，并且会引入明显的同步滞后。
